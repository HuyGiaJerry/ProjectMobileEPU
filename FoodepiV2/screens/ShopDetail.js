...
import ButtonGoBack from "../components/GoBack"; // Nút Back
import { useTheme } from "../context/ThemeContext"; // Import theme context
const ShopDetail = ({ route }) => {
    const { theme } = useTheme(); // Sử dụng theme từ context
    const { shopId } = route.params; // Nhận shopId từ route.params
    const [menuItems, setMenuItems] = useState([]);
    const [loading, setLoading] = useState(true);
    const [selectedItem, setSelectedItem] = useState(null); // Món ăn được chọn
    const [modalVisible, setModalVisible] = useState(false); // Trạng thái modal
    const [quantity, setQuantity] = useState(1); // Số lượng món ăn

    useEffect(() => {
        // Gọi API để lấy toàn bộ danh sách món ăn
        fetch("https://6818be0f5a4b07b9d1d09f95.mockapi.io/dish-by-shop")
            .then((response) => response.json())
            .then((data) => {
                // Lọc dữ liệu theo shopId
    ...
    const openModal = (item) => {
        setSelectedItem(item); // Lưu món ăn được chọn
        setQuantity(1); // Đặt lại số lượng về 1
        setModalVisible(true); // Hiển thị modal
    };

    const closeModal = () => {
        setSelectedItem(null); // Xóa món ăn được chọn
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.background }]}>
            ...
            <View style={styles.restaurantInfo}>
                <Text style={[styles.restaurantName, { color: theme.text }]}>Restaurant #{shopId}</Text>
                <Text style={styles.restaurantDetails}>
                    Explore the best dishes from this restaurant
                </Text>
            </View>
                        ...
                        <View style={[styles.modalContent, { backgroundColor: theme.background }]}>
                            ...
                            <View style={styles.modalActions}>
                                ...
                                <TouchableOpacity
                                    ...
                                    onPress={() => {
                                        console.log(
                                            `Added ${quantity} x ${selectedItem.name} to cart`
                                        );
                                        closeModal();
                                    }}
                                >
